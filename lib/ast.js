...
const {generateJSForms} = require('./generate-js.js');
const {topEnv} = require('./registry.js');
...
class Word {
   ...
   */
  generateJS(scope) {
    if (this.name in topEnv) return this.name;
...
class Apply {
  ...
  generateJS(scope) {
    if (this.operator.type === 'word') {
      ...
      this.operator.type == 'apply' ||
      this.operator.type === 'property'
    ) {
      const argsTranslated = this.args.map((arg) => arg.generateJS(scope));
